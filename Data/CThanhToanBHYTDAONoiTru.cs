...
namespace Data
{
    public class CThanhToanBHYTDAONoiTru
    {
        ...
        public DataSet get_BaoCaoBHYT(string FromDate,string ToDate)
        {
            ...
            sql += "A.TRAITUYEN [TRAITUYEN],[ID] [ID]from thanhtoanbhyt_noitru a,btdbn b where a.mabn=b.mabn and convert(varchar,a.ngaylamviec,112) >='" + for_ngay_yyyymmdd(FromDate) + "' and convert(varchar,a.ngaylamviec,112) <='" + for_ngay_yyyymmdd(ToDate) + "'   ";
        ...
        public string get_IDTTRVdaduyet(string FromDate, string ToDate)
        {
            ...
            try
            {
                ...
                sql = "select convert(varchar,A.IDTTRV) [IDTTRV] from thanhtoanbhyt_noitru a where convert(varchar,a.ngaylamviec,112) >='" + for_ngay_yyyymmdd(FromDate) + "' and convert(varchar,a.ngaylamviec,112) <='" + for_ngay_yyyymmdd(ToDate) + "'   ";
                CConnection data = new CConnection();
                ds = data.getData(sql);
        ...
        public string KiemTraDaDuyet(string MaBN, string IDTTRV, DateTime Ngay)
        {
            ...
            try
            {
                sql = "select [ID] from thanhtoanbhyt_noitru where mabn='"+MaBN+"' and IDTTRV='"+IDTTRV+"' and convert(varchar,NgayLamViec,112)='"+string.Format("{0:yyyyMMdd}", Ngay)+"'";
        ...
        public int Insert(CThanhToanBHYT t)
        {
            ...
            try
            {
                sql = "Insert into thanhtoanbhyt_noitru(mabn,nhomthe,loaithe,mavaovien,sothebhyt,noidangky,icd,chandoan,tongtien,bhyttra,bntra,thuoc,mau,xetnghiem,cdha,dvktthongthuong,dvktcao,vtyt,tienkham,chiphivc,thamdochucnang,giuong,traituyen,idttrv,ngaylamviec,sophieu,khac,manhomthe,ngayhd,loaiba,sobienlai,ngayvao,ngayra)";
                ...
                sql += t.ThamDoChucNang + "," + t.Giuong + "," + t.TraiTuyen + "," + t.IDTTRV + ",'" + FormatYYYY_MM_DD(t.NgayLamViec) + "','" + t.SoPhieu + "'," + t.Khac + ",'" + t.MaNhomThe + "','" + FormatYYYY_MM_DD(t.HSD) + "','" + t.LoaiBA + "'," + t.SoBienLai + ",'" + FormatYYYY_MM_DD(t.NgayVao) + "','" + FormatYYYY_MM_DD(t.NgayRa) + "')";
                CConnection data = new CConnection();
        ...
        {
            sql = "Delete thanhtoanbhyt_noitru where [ID]='" + ID + "'";
            CConnection data = new CConnection();
            data.setData(sql);
        }
        #endregion
